...
bool anamoly(){
    for (int i=0;i<4;i++)
    {
        if (point_1[i][0]<0 || point_1[i][0]>=N || point_1[i][1]>=M)
            return 0;
        else if ((gameGrid[point_1[i][1]][point_1[i][0]])  && (gameGrid[point_1[i][1]][point_1[i][0]] != 10))
        //I have added gameGrid[point_1[i][1]][point_1[i][0]] != 10 so that at no stage shadow point should be treated as a piece...
            return 0;
    }
    return 1;
};
